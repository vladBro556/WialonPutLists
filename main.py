...
def load_params(id,data_from,data_to):#загрузка событий в сессию за период времени
    response = requests.post(f'{HOST}/wialon/ajax.html', params={
...
#Подготовка списка из вложеного словаря для удобства работыс данными
    ...
    a = 'C:/Users/Юзер/Desktop/ex.xlsx'
    for i in list_c:
        b = 'C:/Users/Юзер/Desktop/new/' + i[4] + '.xlsx'
        shutil.copyfile(a, b)
        wb = load_workbook(b)
        ws = wb['data']
        ws['B2'] = v
    ...
    return print('Путевой лист создан','Время на момент записи данных',v,sep='\n')
...
def finish_put_list():
    while True:
        try:
            data_from = int(
            time.mktime(time.strptime(input('Введите время на момент создания путевоголиста в формате ГОД-МЕСЯЦ-ДЕНЬ ЧАС:МИНУТА:СЕКУНДА: '), '%Y-%m-%d %H:%M:%S')))
            data_to = int(
            time.mktime(time.strptime(input('Введите время на момент завешения путевоголиста в формате ГОД-МЕСЯЦ-ДЕНЬ ЧАС:МИНУТА:СЕКУНДА: '), '%Y-%m-%d %H:%M:%S')))
            break
        except:
            ValueError
            print('Вы ввели не правильную дату попробуйте снова')
            continue
    list_a = find_id_and_name_obj(input('Введите назавние транспорта'))
    ...
    for i in list_c:
        s = load_params(i[0],data_from,data_to)
    ...
    for i in list_c:
        ...
        wb = load_workbook(b)
        ws = wb['data']
        ws['C2'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(data_to))
        ws['C3'] = i[4]
        ws['C4'] = i[1]
        ws['C5'] = i[3]
        ws['C6'] = i[2]
    ...
    return print('Создание путевго листа завершено','Время на момент записи данных',v,sep="\n")
...
while True:
    ...
    try:
        if int(q) == 1:
            ...
            print('Вы нажали нету клавишу попробуйте снова')
    except :
        ValueError
        print('Вы нажали букву или символ вместо цифры, попробуйте еще раз.')
